...
func TestRequireAdmin_NonAdminEmail(t *testing.T) {
	...
	rows := sqlmock.NewRows([]string{"id", "email", "name", "username", "password_hash", "is_verified", "is_admin", "admin_role", "password_changed", "disabled_at", "deleted_at", "created_at", "updated_at"}).
		AddRow(userID, "regular@example.com", "Regular User", nil, "hash", true, false, nil, true, nil, nil, time.Now(), time.Now())
...
func TestRequireAdmin_ValidAdmin(t *testing.T) {
	...
	rows := sqlmock.NewRows([]string{"id", "email", "name", "username", "password_hash", "is_verified", "is_admin", "admin_role", "password_changed", "disabled_at", "deleted_at", "created_at", "updated_at"}).
		AddRow(userID, "admin@example.com", "Admin User", nil, "hash", true, true, "superadmin", true, nil, nil, time.Now(), time.Now())
	mock.ExpectQuery(`SELECT (.+) FROM users WHERE id = \$1`).
		WithArgs(userID).
		WillReturnRows(rows)
